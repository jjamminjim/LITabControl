...
- (void)configureSubviews {
    if (_scrollView == nil) {
        ...
        _addButton          = [self buttonWithImageNamed:@"LITabPlusTemplate.pdf" target:self action:@selector(add:)];
        _scrollLeftButton   = [self buttonWithImageNamed:@"LITabLeftTemplate.pdf" target:self action:@selector(goLeft:)];
        _scrollRightButton  = [self buttonWithImageNamed:@"LITabRightTemplate.pdf" target:self action:@selector(goRight:)];
...
- (void)updateButtons {
    ...
    [_addButton.constraints.lastObject setConstant:(showAddButton) ? 48 : 4];
...
- (NSButton *)buttonWithImageNamed:(NSString *)name target:(id)target action:(SEL)action {
    ...
    NSImage* image =[NSImage imageInFrameworkWithFileName:name];
...
- (void)setRemoveAction:(SEL)removeAction {
	if (_removeAction != removeAction) {
...
- (void)removeItem:(id)item {
	[[NSApplication sharedApplication] sendAction:self.removeAction to:self.removeTarget from:item];
...
- (void)reloadData {
    ...
    for (id item in newItems) {
		...
		buttonCell.showsCloseButton = ((self.removeAction != nil) && (self.removeTarget != nil));
